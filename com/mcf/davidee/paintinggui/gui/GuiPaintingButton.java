	...
	public void drawButton(Minecraft mc, int i, int j) {
		if (drawButton && shouldDraw()) {
			...
			if (getHoverState(flag) == 2) { //draw the 4 outlining rectangles
				drawRect(xPosition - BORDER, yPosition - BORDER, xPosition + width + BORDER, yPosition, YELLOW); //upper left to upper right
				drawRect(xPosition - BORDER, yPosition + height, xPosition + width + BORDER, yPosition + height + BORDER, YELLOW); //lower left to lower right
				drawRect(xPosition - BORDER, yPosition, xPosition, yPosition + height, YELLOW); //middle rectangle to the left
				drawRect(xPosition + width, yPosition, xPosition + width + BORDER, yPosition + height, YELLOW); //middle rectangle to the right
			}
		}
	}
